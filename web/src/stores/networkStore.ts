...
import type { Network, Sequencer, NetworksState } from "@/api/types";
...
export const useNetworkStore = create<NetworkStore>((set, get) => ({
  ...
  fetchNetworks: async () => {
    ...
    try {
      ...
      const sortedNetworks = [...networks].sort((a, b) => a.id.localeCompare(b.id));
  ...
  pauseSequencer: async (sequencerId) => {
    try {
      await api.pauseSequencer(sequencerId);
      toast({
  ...
  resumeSequencer: async (sequencerId) => {
    try {
      await api.resumeSequencer(sequencerId);
      toast({
  ...
  haltSequencer: async (sequencerId) => {
    try {
      await api.haltSequencer(sequencerId);
      toast({
