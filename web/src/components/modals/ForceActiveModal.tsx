...
export function ForceActiveModal() {
  const { activeModal, targetSequencerId, closeModal, forceActive, loading, error } = useActionStore();
          ...
          <div className="rounded-md bg-orange-50 dark:bg-orange-900/20 p-4">
            <div className="flex">
              ...
              <div className="text-sm text-orange-800 dark:text-orange-200">
                ...
                <p className="mb-2">This operation bypasses normal sequencer selection and can cause:</p>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="text-sm text-muted-foreground mt-1">
                <p>Sequencer: <span className="font-medium text-foreground">{sequencer.id}</span></p>
                <p>Currently Active: <span className="font-medium text-foreground">{sequencer.sequencer_active ? 'Yes' : 'No'}</span></p>
                <p>Unsafe L2 Block: <span className="font-mono text-foreground">{sequencer.unsafe_l2}</span></p>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Specify a block hash to start from, or leave empty to use the zero hash
              </p>
              {errors.block_hash && (
                <p className="text-sm text-destructive mt-1">{errors.block_hash.message}</p>
              )}
            </div>
          </div>
