...
export function RemoveMemberModal() {
  const { activeModal, targetSequencerId, closeModal, removeMember, loading, error } = useActionStore();
  ...
  const removableMembers = network.sequencers.filter(s => s.id !== sequencer.id);
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                {network.sequencers.length} members ({network.sequencers.filter(s => s.voting).length} voting)
              </p>
            </div>
            ...
            {selectedServerId && (
              <div className="rounded-md bg-muted p-3">
                <p className="text-sm">
                  Selected: <span className="font-mono font-medium">{selectedServerId}</span>
                </p>
                {removableMembers.find(m => m.id === selectedServerId)?.conductor_leader && (
                  <p className="text-sm text-yellow-600 dark:text-yellow-400 mt-1">
                    ⚠️ This member is currently the leader. Consider transferring leadership first.
                  </p>
                )}
              </div>
            )}
