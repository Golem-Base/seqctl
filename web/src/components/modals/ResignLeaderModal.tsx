...
export function ResignLeaderModal() {
  const { activeModal, targetSequencerId, closeModal, resignLeader, loading, error } = useActionStore();
          ...
          <div className="rounded-md bg-yellow-50 dark:bg-yellow-900/20 p-4">
            <div className="flex">
              ...
              <div className="text-sm text-yellow-800 dark:text-yellow-200">
                ...
                <p>This will trigger a new leader election. The cluster will be temporarily without a leader until a new one is elected.</p>
              </div>
            </div>
          </div>

          <div className="text-sm text-muted-foreground">
            <p>Current leader: <span className="font-medium text-foreground">{sequencer.id}</span></p>
            <p>Raft address: <span className="font-mono">{sequencer.raft_addr}</span></p>
          </div>
