...
export function UpdateMembershipModal() {
  const { activeModal, targetSequencerId, closeModal, updateMembership, loading, error } = useActionStore();
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="rounded-md bg-blue-50 dark:bg-blue-900/20 p-4">
            <div className="flex">
              ...
              <div className="text-sm text-blue-800 dark:text-blue-200">
                ...
                <p>Adding a new server to the cluster. Voting members participate in leader election and consensus. Non-voting members receive replicated data but don't vote.</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="mt-2 space-y-1 max-h-32 overflow-y-auto">
                {existingMembers.map((member) => (
                  <div key={member.id} className="text-sm flex items-center justify-between p-2 bg-muted rounded">
            ...
            <div>
              ...
              />
              {errors.server_id && (
                <p className="text-sm text-destructive mt-1">{errors.server_id.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.server_addr && (
                <p className="text-sm text-destructive mt-1">{errors.server_addr.message}</p>
              )}
            </div>
