...
export function TransferLeaderModal() {
  const { activeModal, targetSequencerId, closeModal, transferLeader, loading, error } = useActionStore();
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="rounded-md bg-yellow-50 dark:bg-yellow-900/20 p-4">
            <div className="flex">
              ...
              <div className="text-sm text-yellow-800 dark:text-yellow-200">
                ...
                <p>Leadership transfer will temporarily disrupt consensus operations. Ensure the target sequencer is healthy and properly connected.</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={selectedTargetId} onValueChange={handleTargetSelect}>
                  ...
                  {availableTargets.map((seq) => (
                    <SelectItem key={seq.id} value={seq.id}>
                      <div className="flex items-center justify-between w-full">
                        <span>{seq.id}</span>
                        {seq.voting && (
                          <span className="text-xs text-muted-foreground ml-2">Voting</span>
                        )}
                      </div>
                    </SelectItem>
                  ))}
              ...
              {errors.target_id && (
                <p className="text-sm text-destructive mt-1">{errors.target_id.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.target_addr && (
                <p className="text-sm text-destructive mt-1">{errors.target_addr.message}</p>
              )}
            </div>
          </div>
