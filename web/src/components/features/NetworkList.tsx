...
export function NetworkList() {
  ...
  const [updatedNetworks, setUpdatedNetworks] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    if (networks.length > 0) {
      ...
      networks.forEach(network => {
        const previousNetwork = previousNetworksRef.current.find(n => n.id === network.id);
        
        if (!previousNetwork || 
            previousNetwork.healthy !== network.healthy ||
            previousNetwork.sequencers.length !== network.sequencers.length ||
            previousNetwork.sequencers.filter(s => s.sequencer_active).length !== 
      ...
      if (changedNetworkIds.size > 0 && previousNetworksRef.current.length > 0) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {networks.map((network) => {
          const activeSequencers = network.sequencers.filter(s => s.sequencer_active).length;
          const totalSequencers = network.sequencers.length;
          const leaderSequencer = network.sequencers.find(s => s.conductor_leader);
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      {totalSequencers} sequencer{totalSequencers !== 1 ? 's' : ''}
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground">Active Sequencers:</span>
