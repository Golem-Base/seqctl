...
func (n *Network) Update(ctx context.Context) error {
	...
	for _, seq := range n.sequencers {
		seq := seq
		errg.Go(func() error {
			if err := seq.Initialize(ctx); err != nil {
				return err
			}
	...
	// Wait for all updates to complete and get the first error if any
	err := errg.Wait()
...
func (n *Network) IsHealthy() bool {
	for _, seq := range n.sequencers {
		if !seq.Status.SequencerHealthy {
			return false
		}
	}
	return true
